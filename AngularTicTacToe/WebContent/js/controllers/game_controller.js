...
function GameController($scope, game, grid_size) {
    ...
    $scope.makeMove = function(col, row){
        ...
        if(game.board && game.board.canMove(boardIndex) && !game.winner && !game.tie){
            ...
            // check winner
            if(game.winner) {
                if(game.winner === game.board.X) $scope.status_message = "You Lost !";
                if(game.winner === game.board.O) $scope.status_message = "Player Win ";
                $scope.game_over = true;
            }

            // check tie
            if(game.tie){
                $scope.status_message = "Tie! Draw Game!";
                $scope.game_over = true;
            }
        }
    }
